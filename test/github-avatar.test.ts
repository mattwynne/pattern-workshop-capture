...
for (const selection of ['original', 'cleaned'] as const) {
  test(`GitHub page bundle contains only Markdown and selected ${selection}`, async t => {
    ...
    assert.deepEqual(tree.filter((entry: { path: string }) => entry.path.startsWith("content/")).map((entry: { path: string }) => entry.path), ['content/patterns/pass-the-pen/index.md', 'content/patterns/pass-the-pen/avatar.webp']);
